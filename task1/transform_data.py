...
#for training data
def extract_train():
    f_src = open("complete_train_src.txt", "w")
    f_tgt = open("complete_train_tgt.txt", "w")
...
def extract_dev_and_test():
    switch = 0
    f_dev_src = open("complete_dev_src.txt", "w")
    f_dev_tgt = open("complete_dev_tgt.txt", "w")

    f_test_src = open("complete_test_src.txt", "w")
    f_test_tgt = open("complete_test_tgt.txt", "w")
    for temp_lang in ALL_LANGUAGE_ABBREVIATIONS:
